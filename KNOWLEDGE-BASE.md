...
- **Post-processing — visual realism (Stage 3, 2026-06-02):** `EffectComposer` chain = `RenderPass` → `GTAOPass` (ambient-occlusion contact shadows) → `UnrealBloomPass` (gentle, strength 0.32 / radius 0.5 / threshold 0.82) → `OutputPass`. Toggled by the **Realism FX** panel button (`usePost`). Safety: wrapped in try/catch at init AND per-frame — any failure flips `usePost=false` and falls back to plain `renderer.render`, and the button is disabled. All postprocessing addon URLs + transitive deps were verified at `three@0.160.0` (static-import failure would blank the page, so this matters). **Bump maps:** each procedural texture is reused as a `bumpMap` (`bumpScale 0.06`) so wood/stone/linen show surface relief. NOTE: composer loses MSAA → edges slightly softer with FX on; toggle off for crisp edges.
...
- **Closet cupboards (reworked 2026-06-02):** all four runs are now detailed cupboards built by the **`cupboard(x0,x1,z0,z1,front)`** helper — `front` ∈ `S/N/E/W` picks which face carries the doors (back sits on the opposite wall; a local group is rotated per orientation). Each has a carcass (back/sides/top), vertical dividers into ~2.5 ft modules that are **all hanging (owner: maximize hanging)** — alternating **single full-length hang** and **double-hang (two stories: upper rail + mid shelf + lower rail)** — with a **top storage shelf** across the run, an **interior LED strip** + `roomLight`, and **two-panel sliding TRANSLUCENT doors** (`frost` = `MeshPhysicalMaterial`, opacity 0.34, transmission 0.3) on brass top/bottom tracks so the interior structure shows through. The earlier open display unit + solid runs + toe-kick strips were replaced by this. `ledMat` is emissive, driven by `setMode` (day 0.5 / night 1.7). Frost doors are transparent → skipped by the walk collider.
